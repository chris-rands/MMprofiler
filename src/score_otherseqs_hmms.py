...
def main(ogs_dir, in_file, out_file, evalue, input_suffix):
    true_positive_f_name = os.path.join(ogs_dir, '{}.{}'.format(os.path.basename(in_file).split('.hmm')[0], input_suffix.lstrip('.')))
    other_f_names = [os.path.join(ogs_dir, item) for item in os.listdir(ogs_dir)]
